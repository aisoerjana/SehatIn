...
export default function Result() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .catch(() => loadFromAi());
  }, [result, navigate, macro_target_id]);
    ...
    <div className="page-enter flex flex-col h-screen w-full max-w-md mx-auto bg-[#eff6ff] dark:bg-[#05070d] transition-colors">
      ...
      <div className="flex-1 p-5 overflow-y-auto pb-24">
        ...
        <div className="page-enter-up bg-white dark:bg-[#0b0f17] rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-white/10 transition-colors" style={{ animationDelay: '80ms' }}>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Daily Needs Result</h3>
          <div className="space-y-3">
            <div className="stagger-item flex justify-between items-center p-4 bg-green-50 dark:bg-cyan-400/10 rounded-xl" style={{ animationDelay: '150ms' }}>
              <span className="text-sm font-bold text-gray-800 dark:text-gray-200">Calories</span>
              <span className="text-sm font-bold text-green-700 dark:text-cyan-300">{result.calories} kcal</span>
            </div>
            <div className="stagger-item flex justify-between items-center p-4 bg-blue-50 dark:bg-blue-500/10 rounded-xl" style={{ animationDelay: '210ms' }}>
              ...
              <span className="text-sm font-bold text-blue-700 dark:text-blue-300">{result.protein} g</span>
            </div>
            <div className="stagger-item flex justify-between items-center p-4 bg-orange-50 dark:bg-orange-500/10 rounded-xl" style={{ animationDelay: '270ms' }}>
              <span className="text-sm font-bold text-gray-800 dark:text-gray-200">Carbs</span>
              <span className="text-sm font-bold text-orange-700 dark:text-orange-300">{result.carbs} g</span>
            </div>
            <div className="stagger-item flex justify-between items-center p-4 bg-yellow-50 dark:bg-yellow-500/10 rounded-xl" style={{ animationDelay: '330ms' }}>
              <span className="text-sm font-bold text-gray-800 dark:text-gray-200">Fat</span>
              <span className="text-sm font-bold text-yellow-700 dark:text-yellow-300">{result.fat} g</span>
            </div>
            <div className="stagger-item flex justify-between items-center p-4 bg-red-50 dark:bg-red-500/10 rounded-xl" style={{ animationDelay: '390ms' }}>
              <span className="text-sm font-bold text-gray-800 dark:text-gray-200">Sugar</span>
              <span className="text-sm font-bold text-red-700 dark:text-red-300">{result.sugar} g</span>
            </div>
            <div className="stagger-item flex justify-between items-center p-4 bg-purple-50 dark:bg-purple-500/10 rounded-xl" style={{ animationDelay: '450ms' }}>
              <span className="text-sm font-bold text-gray-800 dark:text-gray-200">Fiber</span>
              <span className="text-sm font-bold text-purple-700 dark:text-purple-300">{result.fiber} g</span>
            </div>
          </div>
        </div>

        {/* Recommended Ingredients + Menu Inspiration */}
        {loading && (
          <div className="page-enter mt-8 flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {macro_target_id ? 'Loading recommendations...' : 'AI is recommending ingredients for you...'}
            </p>
          </div>
        )}
        ...
        {error && (
          <div className="page-enter mt-8 flex items-start gap-2 text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 rounded-2xl px-5 py-4">
            ...
            <div>
              <p className="font-bold mb-0.5">Failed to load recommendations</p>
              <p>{error}</p>
            </div>
          </div>
        )}
        ...
        {recommendation && !loading && (
          <>
            ...
            {recommendation.rekomendasi_bahan?.length > 0 && (
              <div className="page-enter-up mt-8">
                <div className="flex items-center gap-2 mb-4">
                  <Sparkles className="w-5 h-5 text-blue-600 dark:text-cyan-300" />
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Recommended Ingredients</h3>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {recommendation.rekomendasi_bahan.map((item, index) => {
            ...
            {/* Menu Inspiration */}
            {recommendation.inspirasi_menu && (
              <div className="page-enter-up mt-8">
                <div className="flex items-center gap-2 mb-4">
                  <ChefHat className="w-5 h-5 text-blue-600 dark:text-cyan-300" />
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Menu Inspiration</h3>
                </div>
                <div className="bg-white dark:bg-[#0b0f17] rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-white/10 transition-colors">
                  <h4 className="font-bold text-gray-900 dark:text-white text-lg mb-3">
                    {recommendation.inspirasi_menu.judul}
                  </h4>

                  <div className="flex flex-wrap gap-3 mb-4">
                    {recommendation.inspirasi_menu.kalori && (
                      <div className="flex items-center gap-1 text-xs font-bold text-green-700 dark:text-cyan-300">
                        {recommendation.inspirasi_menu.kalori} kcal
                      </div>
                    )}
                    {recommendation.inspirasi_menu.estimasi_harga && (
                  ...
                  {recommendation.inspirasi_menu.bahan?.length > 0 && (
                    <div className="mb-4">
                      <p className="text-xs font-bold text-gray-700 dark:text-gray-300 mb-2">Ingredients:</p>
                      <ul className="space-y-1.5">
                        {recommendation.inspirasi_menu.bahan.map((b, i) => (
                  ...
                  {recommendation.inspirasi_menu.langkah?.length > 0 && (
                    <div className="mb-4">
                      <p className="text-xs font-bold text-gray-700 dark:text-gray-300 mb-2">How to cook:</p>
                      <ol className="space-y-1.5">
                        {recommendation.inspirasi_menu.langkah.map((l, i) => (
                          <li key={i} className="text-xs text-gray-600 dark:text-gray-400 flex items-start gap-2">
                  ...
                  {/* Nutrition */}
                  {(recommendation.inspirasi_menu.protein_g || recommendation.inspirasi_menu.carbs_g || recommendation.inspirasi_menu.lemak_g) && (
                    <div className="flex gap-4 pt-4 border-t border-gray-100 dark:border-white/10">
                      {recommendation.inspirasi_menu.protein_g && (
                        <span className="text-xs font-medium text-red-600 dark:text-red-400">
                          Protein {recommendation.inspirasi_menu.protein_g}g
                        </span>
                      )}
                      {recommendation.inspirasi_menu.carbs_g && (
                        <span className="text-xs font-medium text-yellow-600 dark:text-yellow-400">
                          Carbs {recommendation.inspirasi_menu.carbs_g}g
                        </span>
                      )}
                      {recommendation.inspirasi_menu.lemak_g && (
                        <span className="text-xs font-medium text-orange-600 dark:text-orange-400">
                          Fat {recommendation.inspirasi_menu.lemak_g}g
                        </span>
                      )}
                    </div>
                  )}
